...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	/**
	 * Constant that indicates no external autowiring at all.  表示根本没有外部自动装配的常数
	 * @see #setAutowireMode
	 */
	 ...
	 * through introspection of the bean class.  指示通过bean类的内省来确定适当的自动装配策略常量，
	 * @see #setAutowireMode
	 ...
	 * returned for each call. 返回是否是一个原型，并为每个调用返回一个独立的实例。
	 * @see #SCOPE_PROTOTYPE
	 */
	@Override
	...
	/**
	 * Return the resolved autowire code,  返回解析后的自动装配代码，
	...
	public int getResolvedAutowireMode() {
		if (this.autowireMode == AUTOWIRE_AUTODETECT) {
			// Work out whether to apply setter autowiring or constructor autowiring.  弄清楚是应用setter自动装配还是构造器自动装配。
			// If it has a no-arg constructor it's deemed to be setter autowiring,  如果它有一个无参数的构造函数，它被认为是setter自动装配，
			// otherwise we'll try constructor autowiring.  否则我们将尝试构造函数自动装配。
	 ...
	 * 指定一个回调来创建bean的实例，作为声明式指定的工厂方法的替代方法。<p>如果设置了这样的回调，
	 * 它将覆盖任何其他构造函数或工厂方法元数据。但是，bean属性填充和潜在的注释驱动的注入仍然会像往常一样应用。
	 * @since 5.0
	...
	/**
	 * Return a callback for creating an instance of the bean, if any. 返回一个回调来创建bean的实例，如果有的话
	 * @since 5.0
	 */
	@Nullable
	...
	/**
	 * Return whether to allow access to non-public constructors and methods.  返回是否允许访问非公共构造函数和方法。
	 */
	...
	/**
	 * Return if there are property values values defined for this bean. 返回是否有这个bean定义的属性值。
	 * @since 5.0.2
	 */
	@Override
	 ...
	 * not defined by the application itself. 返回这个bean定义是否是“合成的”，也就是说，不是由应用程序本身定义的。  true：合成的； false：程序本身定义的
	 */
	...
	public void prepareMethodOverrides() throws BeanDefinitionValidationException {
		// Check that lookup methods exist and determine their overloaded status. 检查查找方法是否存在，并确定其重载状态。
